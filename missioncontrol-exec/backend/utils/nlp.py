...
def parse_window(text: str) -> int:
    t = text.lower()
    m = re.search(r"last\s+(\d+)\s*(min|mins|minutes?)", t)
    if m: return int(m.group(1))
    m = re.search(r"last\s+(\d+)\s*(hour|hours|hr|hrs)", t)
    if m: return int(m.group(1)) * 60
    if "last 24 hours" in t: return 24 * 60
    return 60
...
def parse_user_command(text: str) -> dict:
    ...
    if "apigatwey" in t or "apigateway" in t:
        ...
        idx = FLOW_TO_INDEX[flow]
    return {"flow": flow, "index": idx, "window_minutes": parse_window(t), "partner": partner}

def build_where(intent: dict) -> str:
    if intent["flow"] in ("apigateway", "apigatwey"):
        vendor = intent.get("partner") or ""
        return f'(vendor="{vendor}" OR partner="{vendor}")' if vendor else ""
    flow_key = "quick deposit" if intent["flow"] == "quickdeposit" else intent["flow"]
    return FLOW_TO_FILTER.get(flow_key, "")

def build_spl_query(intent: dict):
    index = intent["index"]
    window = intent["window_minutes"]
    where = build_where(intent)
